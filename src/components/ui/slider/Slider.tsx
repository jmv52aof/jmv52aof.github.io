...
const Slider: React.FC<Props> = ({
	...
	if (
		JSON.stringify(Object.keys(items).slice(0).sort()) !=
		JSON.stringify(orderedItemKeys.slice(0).sort())
	) {
		throw new Error(
			'Items keys does not equal to orderedItemKeys, check you parameters'
		)
	}
	...
	const snapToNearestValue = () => {
		if (sliderRef.current && sliderLegendRef.current && pointerRef.current) {
			const legendItems = sliderLegendRef.current.getElementsByClassName(
				styles.legendContainer__text
			)
			const selectedLegendRect =
				legendItems[selectedIndex].getBoundingClientRect()
			const pointerRect = pointerRef.current.getBoundingClientRect()
			const sliderRect = sliderRef.current.getBoundingClientRect()

			if (selectedIndex === keys.length - 1) {
				setPointOffset(
					((sliderRect.width - pointerRect.width / 2) / sliderRect.width) * 100
				)
			} else if (selectedIndex === 0) {
				setPointOffset((pointerRect.width / 2 / sliderRect.width) * 100)
			} else {
				const offsetX =
					selectedLegendRect.left -
					sliderRect.left +
					selectedLegendRect.width / 2
